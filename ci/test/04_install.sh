...
if [ "$NEOSCRYPT" = 1 ]; then
  travis_retry DOCKER_EXEC "apt-get install -y python3-dev && git clone https://github.com/NekosCrypt/neoscrypt-python.git && cd neoscrypt-python && python3 setup.py install --user"
