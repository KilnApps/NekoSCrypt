...
const char * const BITCOIN_CONF_FILENAME = "nekoscrypt.conf";
...
#else
    const char* pszModule = "nekoscrypt";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NekoSCrypt";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NekoSCrypt";
#else
    // Unix
    return pathRet / ".nekoscrypt";
#endif
#endif
}
